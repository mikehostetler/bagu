defmodule Moto.WorkflowSpikeTest do
  ...
  alias MotoTest.WorkflowSpike
  alias MotoTest.WorkflowSpike.{AddAmount, AgentServerAgent}
  alias Runic.Workflow
  alias Runic.Workflow.Invokable

  describe "direct Runic execution" do
    test "runs a Moto.Tool-backed pipeline end-to-end" do
      productions =
  ...
  describe "Jido.Runic.Strategy command loop" do
    test "runs a Moto.Tool-backed pipeline through ExecuteRunnable directives" do
  ...
  describe "AgentServer integration" do
    test "runs the strategy-backed workflow through AgentServer directive execution" do
      jido_name = :"moto_workflow_spike_#{System.unique_integer([:positive])}"
      ...
      {:ok, pid} =
        Jido.AgentServer.start_link(
          ...
          id: "moto-workflow-spike-#{System.unique_integer([:positive])}",
          jido: jido_name,
          debug: true
        )
  ...
  describe "introspection" do
    test "exposes enough graph structure for a future Moto.inspect_workflow/1" do
  ...
  defp make_agent(workflow) do
    agent = %Jido.Agent{
      id: "moto-workflow-spike-#{System.unique_integer([:positive])}",
      name: "moto_workflow_spike",
      description: "Moto workflow spike test agent",
      schema: [],
      state: %{}
    }
