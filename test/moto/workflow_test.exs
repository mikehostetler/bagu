defmodule MotoTest.WorkflowTest do
  use MotoTest.Support.Case, async: false

  @moduletag :capture_log

  alias MotoTest.Workflow.{
    AgentWorkflow,
    EchoAgent,
    FailingWorkflow,
    FunctionWorkflow,
  ...
  test "runs a tool-only workflow through the top-level API" do
    assert {:ok, %{value: 12}} = Moto.Workflow.run(ToolOnlyWorkflow, %{value: 5})
  end
  ...
  test "runs a Moto-compatible agent workflow without live LLM calls" do
  ...
  test "surfaces failing step errors as Moto execution errors" do
  ...
  test "inspects a workflow definition" do
    assert {:ok, inspection} = Moto.inspect_workflow(ToolOnlyWorkflow)
