defmodule Bagu.Demo.Debug do
  ...
  def with_log_level(level, fun) when level in [:info, :debug, :trace] and is_function(fun, 1) do
    previous_debug_level = Bagu.Runtime.debug()
    ...
    try do
      fun.(level)
    after
      Bagu.Runtime.debug(previous_debug_level)
  ...
  def print_trace_status(:trace) do
    status = Bagu.Runtime.debug_status()
  ...
  def print_recent_events(pid, level, opts) when is_pid(pid) and level in [:debug, :trace] do
    ...
    case Bagu.Runtime.recent(pid, limit) do
      {:ok, []} ->
      ...
      {:error, reason} ->
        section(debug_title(level))
        row("events", "failed: #{Bagu.format_error(reason)}")
    end
  ...
  defp apply_log_level(:debug) do
    Logger.configure(level: :warning)
    Bagu.Runtime.debug(:on)
  end

  defp apply_log_level(:trace) do
    Logger.configure(level: :warning)
    Bagu.Runtime.debug(:verbose)
  end

  defp print_request_summary(pid, level, events) do
    case Bagu.Debug.request_summary(pid) do
      {:ok, summary} ->
        section(debug_title(level))
  ...
  defp print_mcp_errors(errors, level) when is_list(errors) do
    Enum.each(errors, fn error ->
      ...
      reason = (error[:message] || Bagu.format_error(error[:reason])) |> preview(level)
  ...
  defp print_memory_summary(%{error: reason}) do
    section("Memory")
    row("error", Bagu.format_error(reason))
  end
  ...
  defp print_error_summary(%{error: {:guardrail, stage, label, reason}}) do
    ...
    row("reason", Bagu.format_error(reason))
  end
  ...
  defp print_error_summary(%{error: other}) do
    if match?({:interrupt, _}, other) do
      ...
      section("Error")
      row("reason", Bagu.format_error(other))
    end
  end
  ...
  defp format_optional_error(error), do: Bagu.format_error(error)
