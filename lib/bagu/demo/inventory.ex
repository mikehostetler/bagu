defmodule Bagu.Demo.Inventory do
  @moduledoc false

  @detail_levels [:debug, :trace]
  @row_width 13

  @spec print_compiled(String.t(), module(), Bagu.Demo.Debug.log_level(), keyword()) :: :ok
  def print_compiled(title, agent_module, level, opts \\ []) when is_atom(agent_module) do
    print_definition(title, agent_module.__bagu__(), level, opts)
  end

  @spec print_imported(String.t(), Bagu.ImportedAgent.t(), Bagu.Demo.Debug.log_level(), keyword()) ::
          :ok
  def print_imported(title, %Bagu.ImportedAgent{} = agent, level, opts \\ []) do
    print_definition(title, Bagu.ImportedAgent.definition(agent), level, opts)
  end

  @spec print_definition(String.t(), map(), Bagu.Demo.Debug.log_level(), keyword()) :: :ok
  ...
  defp format_ash(%{ash_resources: resources}) when is_list(resources) do
    case Bagu.Agent.AshResources.expand(resources) do
