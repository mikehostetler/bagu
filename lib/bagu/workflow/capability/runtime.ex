defmodule Bagu.Workflow.Capability.Runtime do
  @moduledoc false

  @request_meta_key :bagu_workflows

  @doc false
  @spec run_workflow_tool(Bagu.Workflow.Capability.t(), map(), map()) ::
          {:ok, map()} | {:error, term()}
  def run_workflow_tool(%Bagu.Workflow.Capability{} = workflow, params, context)
      when is_map(params) and is_map(context) do
    started_at = System.monotonic_time(:millisecond)
    workflow_context = forwarded_context(context, workflow.forward_context)

    case Bagu.Workflow.run(workflow.workflow, params, context: workflow_context, timeout: workflow.timeout) do
      {:ok, output} ->
  ...
  def run_workflow_tool(%Bagu.Workflow.Capability{} = workflow, _params, context) do
  ...
  defp visible_result(%Bagu.Workflow.Capability{result: :structured}, output, metadata) do
    %{output: output, workflow: visible_metadata(metadata)}
  end

  defp visible_result(%Bagu.Workflow.Capability{}, output, _metadata), do: %{output: output}
  ...
  defp visible_outcome({:error, reason}), do: {:error, Bagu.format_error(reason)}
  ...
  defp normalize_workflow_error(workflow, reason, context) do
    Bagu.Error.Normalize.workflow_error(reason,
  ...
  defp maybe_record_metadata(context, metadata) when is_map(context) and is_map(metadata) do
    parent_server = Map.get(context, Bagu.Subagent.server_key())
    request_id = Map.get(context, Bagu.Subagent.request_id_key())

    if is_pid(parent_server) and is_binary(request_id) do
      Bagu.Workflow.Capability.Metadata.insert(parent_server, request_id, metadata)
    end
  ...
  defp drain_request_meta(server, request_id) when is_pid(server) and is_binary(request_id) do
    Bagu.Workflow.Capability.Metadata.drain(server, request_id)
  end
  ...
  defp lookup_request_meta(server, request_id) when is_pid(server) and is_binary(request_id) do
    Bagu.Workflow.Capability.Metadata.lookup(server, request_id)
  end
  ...
  defp request_id(context) when is_map(context), do: Map.get(context, Bagu.Subagent.request_id_key())
