defmodule Bagu.Workflow.Capability do
  @moduledoc false

  alias Bagu.Workflow.Capability.{Runtime, Tool}
  ...
  @type name :: String.t()
  @type forward_context :: Bagu.Subagent.forward_context()
  ...
  def workflow_names(workflows) when is_list(workflows) do
    ...
    if Enum.uniq(names) == names do
      ...
      {:error, "workflow capability names must be unique within a Bagu agent"}
    end
  end
  ...
  def normalize_available_workflows(other),
    do:
      {:error,
       "available_workflows must be a list of Bagu workflow modules or a map of workflow id => module, got: #{inspect(other)}"}
  ...
  def input_schema(%__MODULE__{workflow: workflow}) do
    {:ok, definition} = Bagu.Workflow.definition(workflow)
  ...
  defp workflow_definition(workflow_module) when is_atom(workflow_module) do
    case Bagu.Workflow.definition(workflow_module) do
      {:ok, definition} ->
        {:ok, definition}

      {:error, reason} ->
        {:error, "workflow #{inspect(workflow_module)} is not a valid Bagu workflow: #{Bagu.format_error(reason)}"}
    end
  end
  ...
  defp normalize_timeout(timeout) do
    case Bagu.Subagent.normalize_timeout(timeout) do
  ...
  defp normalize_forward_context(forward_context) do
    case Bagu.Subagent.normalize_forward_context(forward_context) do
      {:ok, forward_context} ->
        {:ok, forward_context}
