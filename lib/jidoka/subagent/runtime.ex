defmodule Jidoka.Subagent.Runtime do
  ...
  def on_before_cmd(agent, {:ai_react_start, %{request_id: request_id} = params})
    ...
    context =
      ...
      |> Map.put(Jidoka.Subagent.server_key(), self())
      |> Map.put_new(Jidoka.Subagent.depth_key(), current_depth(context))
  ...
  defp visible_result(%Jidoka.Subagent{result: :structured}, result, metadata) do
    %{result: result, subagent: visible_metadata(metadata)}
  end

  defp visible_result(%Jidoka.Subagent{}, result, _metadata), do: %{result: result}
  ...
  defp generated_child_id(%Jidoka.Subagent{name: name}) do
    unique = System.unique_integer([:positive])
    "jidoka-subagent-#{name}-#{unique}"
  end

  defp execute_subagent(%Jidoka.Subagent{} = subagent, params, context) do
  ...
  defp delegate(
         %Jidoka.Subagent{target: :ephemeral} = subagent,
         task,
         _parent_context,
         child_context,
         started_at
  ...
  defp delegate(
         %Jidoka.Subagent{target: {:peer, peer_ref}} = subagent,
         task,
         parent_context,
         child_context,
         started_at
  ...
  defp ask_jidoka_child(agent_module, pid, task, context, timeout) do
    ...
    with {:ok, prepared_opts} <-
           Jidoka.Agent.prepare_chat_opts(child_opts, child_chat_config(agent_module)),
         request_opts <-
           Keyword.merge(
             prepared_opts,
  ...
  defp normalize_jidoka_child_result(await_result, pid, request_id, _timeout) do
    result =
      pid
      |> Jidoka.finalize_chat_request(request_id, await_result)
  ...
  defp resolve_peer_pid(peer_id) when is_binary(peer_id) do
    case Jidoka.whereis(peer_id) do
  ...
  defp forwarded_context(context, policy) do
    ...
    |> Map.put(Jidoka.Subagent.depth_key(), current_depth(context) + 1)
  end

  defp current_depth(context) when is_map(context) do
    case Map.get(context, Jidoka.Subagent.depth_key(), 0) do
  ...
  defp normalize_subagent_error(subagent, reason, context, metadata) do
    Jidoka.Error.Normalize.subagent_error(reason,
      ...
      target: subagent.target,
      request_id: Map.get(context, Jidoka.Subagent.request_id_key()) || Map.get(metadata, :child_request_id),
      cause: reason
    )
  end
