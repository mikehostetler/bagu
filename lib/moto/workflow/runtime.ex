defmodule Moto.Workflow.Runtime do
  @moduledoc false
  ...
  @definition_key :__moto_workflow_definition__
  @step_key :__moto_workflow_step__
  @state_key :__moto_workflow_state__
  ...
  defp action_node(definition, step) do
    ActionNode.new(
      Moto.Workflow.StepAction,
      %{
        @definition_key => definition,
        @step_key => step
      },
      name: step.name,
      inputs: [{@state_key, [type: :any, doc: "Moto workflow runtime state"]}],
      outputs: [{@state_key, [type: :any, doc: "Moto workflow runtime state"]}],
      timeout: 0,
      log_level: :warning,
      max_retries: 0
    )
  ...
  defp normalize_opts(opts) do
    with {:ok, context} <- Moto.Context.normalize(Keyword.get(opts, :context, %{})),
  ...
  defp normalize_agents(agents) when is_list(agents) do
    if Keyword.keyword?(agents) do
      ...
      {:error, Moto.Error.validation_error("Invalid workflow agents: pass `agents:` as a map or keyword list.")}
    end
  end

  defp normalize_agents(other) do
    {:error,
     Moto.Error.validation_error("Invalid workflow agents: pass `agents:` as a map or keyword list.",
       field: :agents,
       value: other,
  ...
  defp normalize_timeout(other) do
    {:error,
     Moto.Error.validation_error("Invalid workflow timeout: expected a positive integer.",
       field: :timeout,
       value: other,
  ...
  defp normalize_return(other) do
    {:error,
     Moto.Error.validation_error("Invalid workflow return option: expected `:output` or `:debug`.",
       field: :return,
       value: other,
  ...
  defp coerce_input_map(input) do
    case Moto.Context.coerce_map(input) do
      {:ok, map} ->
        {:ok, map}

      :error ->
        {:error,
         Moto.Error.validation_error("Invalid workflow input: pass input as a map or keyword list.",
           field: :input,
           value: input,
           details: %{reason: :expected_map}
         )}
  ...
  defp do_parse_input(definition, input_map) do
    case Zoi.parse(definition.input_schema, input_map) do
      ...
      {:ok, other} ->
        {:error,
         Moto.Error.config_error("Workflow input schema must parse to a map.",
           field: :input_schema,
           value: other,
      ...
      {:error, errors} ->
        {:error,
         Moto.Error.validation_error("Invalid workflow input:\n" <> Zoi.prettify_errors(errors),
           field: :input,
           value: input_map,
  ...
  defp validate_context_refs(definition, context) do
    Enum.reduce_while(definition.context_refs, :ok, fn key, :ok ->
      if has_equivalent_key?(context, key) do
        ...
        {:halt,
         {:error,
          Moto.Error.validation_error("Missing workflow context key `#{key}`.",
            field: :context,
            value: context,
  ...
  defp validate_imported_agent_refs(definition, agents) do
    Enum.reduce_while(definition.imported_agent_refs, :ok, fn key, :ok ->
      if has_equivalent_key?(agents, key) do
        ...
        {:halt,
         {:error,
          Moto.Error.validation_error("Missing imported workflow agent `#{key}`.",
            field: :agents,
            value: agents,
  ...
  defp run_strategy(definition, state, runtime_opts) do
    ...
    agent = %Jido.Agent{
      id: "moto-workflow-#{definition.id}-#{System.unique_integer([:positive])}",
      name: definition.id,
      description: definition.description || "Moto workflow #{definition.id}",
      schema: [],
      state: %{}
    }

    strategy_context = %{agent_module: Moto.Workflow.StepAction, strategy_opts: [workflow: workflow]}
  ...
  defp run_agent_target(pid, prompt, context, timeout) when is_pid(pid) do
    call_agent(fn -> Moto.chat(pid, prompt, context: context, timeout: timeout) end, timeout)
  end

  defp run_agent_target(%Moto.ImportedAgent{} = agent, prompt, context, timeout) do
    run_started_agent(fn opts -> Moto.start_agent(agent, opts) end, fn pid ->
      call_agent(fn -> Moto.chat(pid, prompt, context: context, timeout: timeout) end, timeout)
    end)
  end
  ...
  defp run_started_agent(start_fun, call_fun) do
    child_id = "moto-workflow-agent-#{System.unique_integer([:positive])}"
  ...
  defp extract_state_from_fact!(other) do
    raise ArgumentError, "expected Moto workflow state fact, got: #{inspect(other)}"
  end
  ...
  defp merge_states([]), do: raise(ArgumentError, "expected at least one Moto workflow state")
  ...
  defp resolve_value({:moto_workflow_ref, :input, key}, state), do: fetch_ref(state.input, key, :input)
  defp resolve_value({:moto_workflow_ref, :context, key}, state), do: fetch_ref(state.context, key, :context)
  defp resolve_value({:moto_workflow_ref, :value, value}, _state), do: {:ok, value}

  defp resolve_value({:moto_workflow_ref, :from, step, nil}, state), do: fetch_ref(state.steps, step, :step)

  defp resolve_value({:moto_workflow_ref, :from, step, path}, state) when is_list(path) do
  ...
  defp step_error(definition, step, reason) do
    Moto.Error.execution_error("Workflow #{definition.id} step #{step.name} failed.",
      phase: :workflow_step,
      details: %{
        workflow_id: definition.id,
        step: step.name,
