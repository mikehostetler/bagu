defmodule Moto.Skill do
  @moduledoc false

  alias Jido.AI.Skill

  @context_key :__moto_skills__
  @state_key :__moto_skill_runtime__
  ...
  def normalize_dsl(entries, base_dir) when is_list(entries) and is_binary(base_dir) do
    ...
    |> Enum.reduce_while({:ok, %{refs: [], load_paths: []}}, fn
      %Moto.Agent.Dsl.SkillRef{skill: skill}, {:ok, acc} ->
      ...
      %Moto.Agent.Dsl.SkillPath{path: path}, {:ok, acc} ->
  ...
  defp ensure_unique_registry_name(name, acc) do
    if Map.has_key?(acc, name) do
      {:error, "skill names must be unique within a Moto skill registry"}
    else
      :ok
    end
  end
