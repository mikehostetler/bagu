defmodule Moto.Debug do
  @moduledoc false

  alias Jido.AI.Request

  @meta_key :moto_debug
  @pending_table :moto_debug_requests
  @type summary :: %{
          request_id: String.t(),
          status: atom() | nil,
          model: term(),
          ...
          interrupt: Moto.Interrupt.t() | nil,
          error: term() | nil,
      ...
      {:ok, %{agent: agent}} ->
        ...
        case Request.get_request(agent, request_id) do
          ...
          request ->
            ...
            subagent_calls = Moto.Subagent.request_calls(server_or_id, request_id)
  ...
  defp build_summary(agent, request_id, request, pending_meta, subagent_calls) do
    ...
    hook_meta = Map.get(request_meta, :moto_hooks, %{})
    guardrail_meta = Map.get(request_meta, :moto_guardrails, %{})
    memory_meta = Map.get(request_meta, :moto_memory, %{})

    interrupt =
      case request.error do
        {:interrupt, %Moto.Interrupt{} = interrupt} -> interrupt
  ...
  defp maybe_store_runtime_meta(runtime_context, attrs) when is_map(runtime_context) and is_map(attrs) do
    case {resolve_server(Map.get(runtime_context, Moto.Subagent.server_key())),
          Map.get(runtime_context, Moto.Subagent.request_id_key())} do
  ...
  defp stored_subagent_calls(agent, request_id) do
    case Moto.Subagent.get_request_meta(agent, request_id) do
