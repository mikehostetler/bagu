defmodule Moto.Agent.AshResources do
  @moduledoc false
    ...
    cond do
      ...
      true ->
        with {:ok, _domain} <- domain(resource),
             ...
             {:ok, _names} <- Moto.Tool.action_names(actions) do
          :ok
        end
    end
  rescue
  ...
  def expand(resources) when is_list(resources) do
    Enum.reduce_while(resources, {:ok, initial_acc()}, fn resource, {:ok, acc} ->
      with :ok <- validate_resource(resource),
           ...
           {:ok, resource_tool_names} <- Moto.Tool.action_names(resource_actions),
  ...
  defp ensure_same_domain(expected, actual) do
    {:error,
     "Moto ash_resource tools must all belong to the same Ash domain; got #{inspect(expected)} and #{inspect(actual)}"}
  end
