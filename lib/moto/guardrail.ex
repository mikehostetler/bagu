defmodule Moto.Guardrail do
  @moduledoc """
  Thin wrapper for reusable Moto guardrails.

  Moto guardrails are published by name and expose a single `call/1` callback.
  They can be referenced from the Moto DSL, imported JSON/YAML specs, or
  ...
  defp ensure_unique_registry_name(name, acc) do
    if Map.has_key?(acc, name) do
      {:error, "guardrail names must be unique within a Moto guardrail registry"}
    else
      :ok
    end
  end
