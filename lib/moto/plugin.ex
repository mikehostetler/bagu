defmodule Moto.Plugin do
  @moduledoc """
  Thin wrapper around `Jido.Plugin` for defining Moto-friendly plugins.
  ...
  Moto plugins are the main extension point for adding capability around a Moto
  ...
  @typedoc """
  A published Moto plugin name.
  """
  @type name :: String.t()

  @typedoc """
  A registry of published Moto plugin names to plugin modules.
  """
  ...
  defmacro __using__(opts \\ []) do
    ...
    defaults = [
      ...
      description: "Moto plugin #{module_name}",
      actions: [],
  ...
  @doc """
  Validates that a module behaves like a Moto plugin.
  """
  ...
  @doc """
  Returns the published name for a validated Moto plugin module.
  """
  ...
  @doc """
  Returns the published names for a list of Moto plugin modules.
  """
    ...
    |> case do
      {:ok, names} ->
        if Enum.uniq(names) == names do
          ...
          {:error, "plugin names must be unique within a Moto agent"}
        end
  ...
  def plugin_actions(module) when is_atom(module) do
    with :ok <- ensure_compiled_plugin(module),
         ...
         {:ok, _action_names} <- Moto.Tool.action_names(actions) do
      {:ok, actions}
    else
      {:error, reason} ->
        {:error, reason}
  ...
  def normalize_available_plugins(other),
    do:
      {:error,
       "available_plugins must be a list of Moto plugin modules or a map of name => module, got: #{inspect(other)}"}
